...
function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      let query = supabase
        .from('revenue')
        .select('amount, date, status') // Include status in the select
      ...
      if (revenueError) {
        ...
        // Calculate total confirmed revenue (status = Completed)
        const revenueStatusCompleted = revenueData.filter(item => item.status === 'Completed');
        const confirmedRevenue = revenueStatusCompleted.reduce((acc, curr) => {
  ...
  const aggregateData = (data, timeRange) => {
    ...
    data.forEach(item => {
      if (item.status !== 'Completed') {
        return; // Skip if status is not Completed
      }
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white shadow-md rounded-lg p-4">
              <h2 className="text-xl font-semibold mb-2">Confirmed Revenue</h2>
