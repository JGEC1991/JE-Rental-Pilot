import React, { useState, useEffect, useRef } from 'react';
    ...
    function ActivityRecordCard({ activity, isEditMode = false, onClose, onActivityUpdated }) {
      ...
      const [activityType, setActivityType] = useState(activity?.activity_type || activityTypes[0]);
      const [description, setDescription] = useState(activity?.description || '');
      const [date, setDate] = useState(activity?.date || new Date().toISOString().split('T')[0]);
      const [attachments, setAttachments] = useState(activity?.attachment_url || '');
      ...
      const handleAttachmentUpload = async (e) => {
        ...
        // Update local state
        setAttachments(validUrls.length > 0 ? validUrls[0] : '');
        
        // If in edit mode, also update the activity in the database
        if (isEditMode && activity?.id) {
          const { error } = await supabase
            .from('activities')
            .update({ attachment_url: validUrls.length > 0 ? validUrls[0] : null })
            .eq('id', activity.id);
      ...
      const handleSave = async () => {
        try {
          ...
          console.log("Saving activity with attachments:", updatedActivity.attachment_url);
            ...
            {attachments && (
              <div>
                <img
                  ...
                  onClick={() => handleImageClick(attachments)}
                />
              </div>
            )}
          </div>
