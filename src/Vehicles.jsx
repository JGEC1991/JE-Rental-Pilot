import { useState, useEffect } from 'react'
    ...
    const Vehicles = () => {
      ...
      const columns = [
        { key: 'make', title: t('make', { ns: 'vehicles' }) },
        { key: 'model', title: t('model', { ns: 'vehicles' }) },
        { key: 'year', title: t('year', { ns: 'vehicles' }) },
        { key: 'color', title: t('color', { ns: 'vehicles' }) },
        { key: 'license_plate', title: t('licensePlate', { ns: 'vehicles' }) },
        { key: 'vin', title: t('vin', { ns: 'vehicles' }) },
        { key: 'status', title: t('status', { ns: 'vehicles' }) },
      ]
      ...
      return (
        <div className="container mx-auto p-6">
          ...
          <Popout isOpen={showAddForm} onClose={handleCloseAddForm}>
            <h2 className="text-xl font-semibold mb-4">{selectedVehicle ? t('editVehicle', { ns: 'vehicles' }) : t('addNewVehicle', { ns: 'vehicles' })}</h2>
            ...
            <form onSubmit={handleAddVehicleSubmit} className="max-w-lg">
              {activeTab === 'information' && (
                <div>
                  <div className="mb-4">
                    ...
                    <input type="text" id="make" className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value={newVehicle.make} onChange={handleInputChange} placeholder={t('enterMake', { ns: 'vehicles' })} />
                  </div>
                  <div className="mb-4">
                    <label htmlFor="model" className="block text-gray-700 text-sm font-bold mb-2">{t('model', { ns: 'vehicles' })}</label>
                    <input type="text" id="model" className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value={newVehicle.model} onChange={handleInputChange} placeholder={t('enterModel', { ns: 'vehicles' })} />
                  </div>
                  <div className="mb-4">
                    <label htmlFor="year" className="block text-gray-700 text-sm font-bold mb-2">{t('year', { ns: 'vehicles' })}</label>
                    <input type="number" id="year" className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value={newVehicle.year} onChange={handleInputChange} placeholder={t('enterYear', { ns: 'vehicles' })} />
                  </div>
                  <div className="mb-4">
                    <label htmlFor="color" className="block text-gray-700 text-sm font-bold mb-2">{t('color', { ns: 'vehicles' })}</label>
                    <input type="text" id="color" className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value={newVehicle.color} onChange={handleInputChange} placeholder={t('enterColor', { ns: 'vehicles' })} />
                  </div>
                  <div className="mb-4">
                    <label htmlFor="license_plate" className="block text-gray-700 text-sm font-bold mb-2">{t('licensePlate', { ns: 'vehicles' })}</label>
                    <input type="text" id="license_plate" className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value={newVehicle.license_plate} onChange={handleInputChange} placeholder={t('enterLicensePlate', { ns: 'vehicles' })} />
                  </div>
                  <div className="mb-4">
                    <label htmlFor="vin" className="block text-gray-700 text-sm font-bold mb-2">VIN</label>
                    <input type="text" id="vin" className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value={newVehicle.vin} onChange={handleInputChange} placeholder={t('enterVIN', { ns: 'vehicles' })} />
                  </div>
                  <div className="mb-4">
                    ...
                    <select id="status" className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value={newVehicle.status} onChange={handleInputChange}>
                      <option value="available">{t('available', { ns: 'vehicles' })}</option>
                      <option value="rented">{t('rented', { ns: 'vehicles' })}</option>
                      <option value="maintenance">{t('maintenance', { ns: 'vehicles' })}</option>
                    </select>
                  </div>
                  <div className="mb-4">
                    <label htmlFor="observations" className="block text-gray-700 text-sm font-bold mb-2">{t('observations', { ns: 'vehicles' })}</label>
                    <textarea id="observations" className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value={newVehicle.observations} onChange={handleInputChange} placeholder={t('enterObservations', { ns: 'vehicles' })} />
                  </div>
                </div>
              )}
              {activeTab === 'photos' && (
              ...
              <div className="flex items-center justify-end">
                <button type="submit" className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">{selectedVehicle ? t('edit', { ns: 'vehicles' }) : t('addVehicle', { ns: 'vehicles' })}</button>
              </div>
            </form>
          </Popout>

          <Popout isOpen={showViewForm} onClose={handleCloseViewForm}>
            <h2 className="text-xl font-semibold mb-4">{t('vehicleDetails', { ns: 'vehicles' })}</h2>
            <div>
              <p><strong>{t('make', { ns: 'vehicles' })}:</strong> {selectedVehicle?.make}</p>
              <p><strong>{t('model', { ns: 'vehicles' })}:</strong> {selectedVehicle?.model}</p>
              <p><strong>{t('year', { ns: 'vehicles' })}:</strong> {selectedVehicle?.year}</p>
              <p><strong>{t('color', { ns: 'vehicles' })}:</strong> {selectedVehicle?.color}</p>
              <p><strong>{t('licensePlate', { ns: 'vehicles' })}:</strong> {selectedVehicle?.license_plate}</p>
